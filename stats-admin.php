...
function crunchReportData(games, title, subtitle, scope) {
    ...
    periods.forEach(p => {
        ...
        boxScore[p] = {
            waveGoals:  pEvts.filter(e => ['goal','penalty_5m_goal_for'].includes(e.stat) && playerMap[e.playerId]).length,
            oppGoals:   pEvts.filter(e => ['goals_against','penalty_5m_goal_against'].includes(e.stat)).length,
            waveShots:  pEvts.filter(e => ['goal','shot'].includes(e.stat) && playerMap[e.playerId]).length,
            waveSaves:  pEvts.filter(e => e.stat === 'save' && playerMap[e.playerId]).length,
        };
    });
    ...
    // ── 3. Player Stat Leaderboards ─────────────────────────────────────────
    const statKeys = ['goal','assist','shot','steal','block','turnover','kickout','kickout_earned','save'];
    const playerStats = {}; // pid → { name, number, isGoalie, stats:{}, shotsTaken, goalsScored, saves, shotsAgainst }
    ...
    // ── 5. Man-Up / Man-Down Shooting ───────────────────────────────────────
    const shotEvts = allEvents.filter(e => ['goal','shot'].includes(e.stat) && playerMap[e.playerId]);
    const situations = {
    ...
    // ── 9. Game Flow (goals by period per game) ──────────────────────────────
    ...
    const gameFlow = gameResults.map(g => {
        ...
        flowPeriods.forEach(p => {
            ...
            pMap[p] = {
                wave: pEvts.filter(e => ['goal','penalty_5m_goal_for'].includes(e.stat) && playerMap[e.playerId]).length,
    ...
    data.sections.forEach(sec => {
        ...
        if (sec.type === 'results_table') {
            ...
            if (allP.length) {
                ...
                cols.forEach(k => html += `<th style="padding:5px 8px">${{goal:'G',assist:'A',shot:'SH',steal:'ST',block:'BL',turnover:'TO',kickout:'KO',kickout_earned:'KOE',save:'SV'}[k]||k}</th>`);
                html += `</tr></thead><tbody>`;
                allP.forEach((p,i) => {
                    const bg = i%2===0?'#f8f9fc':'#fff';
            ...
            Object.entries(sec.leaders).forEach(([stat, players]) => {
                ...
                players.forEach((p,i) => {
                    ...
                    html += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;font-size:12px">
                        ...
                        <span style="font-family:var(--fm);font-weight:700;color:var(--navy)">${p.stats[stat]}</span>
                    </div>`;
                });
                html += `</div>`;
            });
